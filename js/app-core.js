(() => {
  ...
  function avatarFallback(user) { return (user?.displayName || user?.email || 'M').trim().charAt(0).toUpperCase(); }
  ...
  function renderAccount() {
    ...
    document.querySelectorAll('[data-user-photo]').forEach((element) => {
      ...
      if (element.tagName === 'IMG') { if (source) { element.src = source; element.hidden = false; } else { element.hidden = true; } }
      else element.textContent = avatarFallback(user);
    });
  }
  ...
  function closeMenus() {
    ...
    links?.classList.remove('open'); toggle?.setAttribute('aria-expanded', 'false'); if (profileMenu) profileMenu.hidden = true;
  }
  ...
  function bindNavigation() {
    const navToggle = document.querySelector('[data-nav-toggle]'); const links = document.querySelector('[data-nav-links]');
    const profileButton = document.querySelector('[data-profile-toggle]'); const profileMenu = document.querySelector('[data-profile-menu]');
    navToggle?.addEventListener('click', (event) => { event.stopPropagation(); if (profileMenu) profileMenu.hidden = true; const open = !links?.classList.contains('open'); links?.classList.toggle('open', open); navToggle.setAttribute('aria-expanded', String(open)); });
    profileButton?.addEventListener('click', (event) => { event.stopPropagation(); links?.classList.remove('open'); navToggle?.setAttribute('aria-expanded','false'); if (profileMenu) profileMenu.hidden = !profileMenu.hidden; });
    document.addEventListener('click', (event) => { if (!event.target.closest('.mh-profile-wrap') && !event.target.closest('[data-nav-links]')) closeMenus(); });
    document.addEventListener('keydown', (event) => { if (event.key === 'Escape') closeMenus(); });
    document.querySelectorAll('[data-nav-links] a,[data-profile-menu] a').forEach((link) => { const close = () => { closeMenus(); if (!link.href.includes('#')) showLoader('Opening page…'); }; link.addEventListener('pointerdown', close); link.addEventListener('click', close); });
    document.querySelectorAll('[data-logout]').forEach((button) => button.addEventListener('click', signOut));
    window.addEventListener('pageshow', (event) => { closeMenus(); if (event.persisted || state.ready) reveal(); }); window.addEventListener('pagehide', closeMenus); window.addEventListener('resize', closeMenus); document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'hidden') closeMenus(); });
  }
  ...
  window.MOBAHub = { state, init, onReady(listener) { state.ready ? listener(state) : listeners.add(listener); }, toast, escapeHtml, signOut, renderAccount, closeMenus, url, validNext, LOGIN_ROUTE, HOME_ROUTE, ROOT_ROUTE: paths.root };
