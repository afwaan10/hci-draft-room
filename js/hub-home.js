(() => {
  ...
  function renderAuth(user){
    ...
    if(currentUser){
      ...
      if(els.userEmail) els.userEmail.textContent = currentUser.email || 'Google account';
      if(els.userPhoto){
  ...
  async function loadHistory(){
    ...
    try{
      ...
      els.historyList.innerHTML = rows.slice(0, 12).map((row) => {
        ...
        const href = `/${String(game).toLowerCase()}/?room=${encodeURIComponent(room)}`;
        return `<article class="history-item"><div><strong>${escapeHtml(game)} · ${escapeHtml(roomName)}</strong><span>${escapeHtml(row.teamAName || 'Team A')} vs ${escapeHtml(row.teamBName || 'Team B')}</span><small>${escapeHtml(room)} · Game ${Number(row.gameNumber || 1)} / BO${Number(row.bestOf || 3)} · ${escapeHtml(date)}</small></div><a class="tiny-btn" href="${href}">Open</a></article>`;
      }).join('');
    }catch(error){
